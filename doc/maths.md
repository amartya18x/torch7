...
The `output` `Tensor` that is fed into the `batchdraw` method need not be contiguous and it can have any shape. The shape of the `Tensor` after the execution of the function will not change and it will be filled with sampled indices. This method is exceptionally faster than `torch.multinomial` when you want to sample a lot of samples from the same distrbution or sample from the same distribution a large number of times. `torch.multinomial` is faster for sampling few samples from a distribution once because the `setup` of the Alias takes some time in this case. To see and compare how these two methods differ in speed run `th test/test_aliasMultinomial.lua`.
